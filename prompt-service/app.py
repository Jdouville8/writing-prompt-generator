...
def generate():
    ...
    with tracer.start_as_current_span("generate-prompt") as span:
        try:
            ...
            # Generate cache key
            cache_key = f"prompt:{':'.join(sorted(genres))}"
            cache_key_hash = hashlib.md5(cache_key.encode()).hexdigest()
            
            # Check cache
            cached_prompt = redis_client.get(cache_key_hash)
            if cached_prompt and random.random() > 0.3:  # 70% chance to use cache
            ...
            # Cache the result (with some randomness to ensure variety)
            if random.random() > 0.5:  # 50% chance to cache
                redis_client.setex(
                    cache_key_hash,
                    3600,  # 1 hour TTL
                    json.dumps(prompt)
